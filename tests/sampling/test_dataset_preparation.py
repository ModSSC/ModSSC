...
from modssc.sampling import SamplingPlan, prepare_dataset
from modssc.sampling.dataset import _concat_rows


def _plan(*, merge: bool) -> SamplingPlan:
    return SamplingPlan.from_dict({"policy": {"merge_official_splits": merge}})
...
def test_prepare_dataset_is_identity_without_merge() -> None:
    dataset = _dataset()
    assert prepare_dataset(dataset, plan=_plan(merge=False)) is dataset


def test_prepare_dataset_merges_official_splits_and_derives_identity() -> None:
    first = prepare_dataset(_dataset(), plan=_plan(merge=True))
    second = prepare_dataset(_dataset(), plan=_plan(merge=True))
