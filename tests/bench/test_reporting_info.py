...
from tools.hpc.daily_report import _run_measurements
...
def test_run_summary_normalizes_linux_rss_and_daily_report_consumes_it(
    ...
    observed = _run_measurements(run_json)
    assert observed["peak_ram_bytes"] == pytest.approx(3072 * 1024)
    assert observed["peak_ram_source"] == "peak_ram_bytes"
...
def test_cuda_peaks_are_reset_synchronized_and_report_reserved_bytes(
    ...
    observed = _run_measurements(run_json)
    assert observed["peak_vram_bytes"] == pytest.approx(4096)
    assert observed["peak_vram_source"] == "peak_vram_bytes"
