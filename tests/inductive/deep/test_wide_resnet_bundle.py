...
def test_wide_resnet_bundle_paper_defaults() -> None:
    ...
    assert {key: value for key, value in bundle.meta.items() if key not in hook_names} == {
    ...
    assert all(callable(bundle.meta[name]) for name in hook_names)
    assert bundle.contract is not None
    assert hook_names | {"logits"} <= bundle.contract.outputs

    features = bundle.meta["forward_features"](sample)
    assert features.ndim == 2
    assert bundle.meta["forward_head"](features).shape == (2, 10)
