...
from modssc.evaluation import make_inductive_split_provider
...
def test_views_for_split_keeps_numpy_views_for_numpy_backend() -> None:
    ...
    preprocess = SimpleNamespace(
        dataset=SimpleNamespace(
            train=SimpleNamespace(X=data.X_l, y=data.y_l),
            test=None,
        ),
    ...
    sampling = _Sampling()
    sampling.is_graph = lambda: False
    payload = make_inductive_split_provider(
...
def test_democratic_combine_scores_numpy_eligibility():
    ...
    with pytest.raises(InductiveValidationError, match="no learner above min_confidence"):
