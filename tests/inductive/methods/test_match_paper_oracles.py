...
def test_torchssl_historical_evaluation_switches_to_tail_cadence() -> None:
    ...
    assert 839000 in scheduled
    assert 1_045_000 in scheduled
    assert 1_048_000 in scheduled
...
def test_native_checkpoint_preserves_sampler_resume_trajectory(tmp_path: Path) -> None:
    identity = {"method_id": "fixmatch", "contract_sha256": "b" * 64}
    ...
    store = match_trainer._CheckpointStore(identity=identity, context=context)
    ...
    assert store.store.pointer_path.is_file()
    assert len(list(store.store.generations_dir.iterdir())) == 1
    recovered_store = match_trainer._CheckpointStore(identity=identity, context=context)
