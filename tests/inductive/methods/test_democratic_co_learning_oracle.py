...
def test_democratic_figure2_all_filtered_fallback_is_nonzero_and_backend_exact(
    weights: np.ndarray,
) -> None:
    predictions = np.asarray([[1, 0], [1, 0], [0, 1]], dtype=np.int64)
    numpy_scores = dcl._combine_scores_numpy(
        predictions,
        weights,
        n_classes=2,
        min_confidence=0.5,
    )
    assert np.all(numpy_scores.sum(axis=1) > 0.0)

    torch = pytest.importorskip("torch")
    torch_scores = dcl._combine_scores_torch(
        torch.as_tensor(predictions),
        torch.as_tensor(weights),
        n_classes=2,
        min_confidence=0.5,
    )
    np.testing.assert_allclose(torch_scores.numpy(), numpy_scores, rtol=0.0, atol=1e-12)
