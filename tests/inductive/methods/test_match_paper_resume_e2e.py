...
def _synthetic_data() -> InductiveDataset:
    ...
    return InductiveDataset(
        ...
        meta={
            ...
            "paper_evaluation": {
                "X_test": x_test,
                "y_test": torch.tensor([0, 1, 0, 1], dtype=torch.int64),
            },
        },
    )
...
def _checkpoint(root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
...
def test_paper_match_interrupted_resume_is_bit_exact(
    ...
    data = _synthetic_data()
    monkeypatch.setenv("MODSSC_TASK_ID", "match-resume-e2e")
    monkeypatch.setenv("MODSSC_CHECKPOINT_IDENTITY_SHA256", "match-resume-identity")
    monkeypatch.setenv("MODSSC_EXPECTED_GIT_SHA", "match-resume-commit")
    monkeypatch.setenv("MODSSC_ENVIRONMENT_LOCK_SHA256", "match-resume-environment")
    ...
    monkeypatch.setenv("MODSSC_CHECKPOINT_ROOT", str(continuous_root))
    ...
    continuous_result = continuous_run(data)
    continuous_checkpoint, continuous_metadata = _checkpoint(continuous_root)

    resumed_root = tmp_path / "resumed"
    monkeypatch.setenv("MODSSC_CHECKPOINT_ROOT", str(resumed_root))
    seed_everything(314159, deterministic=True)
    ...
    with pytest.raises(PlannedContinuation):
        interrupted_run(data)
    interrupted_checkpoint, interrupted_metadata = _checkpoint(resumed_root)
    ...
    monkeypatch.setattr(match_trainer, "_continuation_requested", lambda: False)
    resumed_result = resumed_run(data)
    resumed_checkpoint, resumed_metadata = _checkpoint(resumed_root)
    ...
    assert [event["reason"] for event in continuous_metadata["history"]] == [
        "periodic",
        "periodic",
        "complete",
    ]
    assert [event["reason"] for event in resumed_metadata["history"]] == [
        "periodic",
