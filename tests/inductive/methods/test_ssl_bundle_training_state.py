...
def test_match_evaluation_contract_reports_ema_as_primary_model(method_cls: type) -> None:
    bundle = _training_bundle()
    method = _method(method_cls, bundle)
    method.fit(_dataset(), device=DeviceSpec(device="cpu"), seed=5)
    with torch.no_grad():
        bundle.model.weight.zero_()
        bundle.model.bias.copy_(torch.tensor([4.0, 0.0]))
        bundle.ema_model.weight.zero_()
        bundle.ema_model.bias.copy_(torch.tensor([0.0, 4.0]))
    features = torch.zeros(3, 2)
    selected = InductiveEvaluationSplit(
        X=features,
        y_true=torch.ones(3, dtype=torch.int64),
    )

    assert torch.all(method.predict_proba(features).argmax(dim=1) == 0)
    result = evaluate_inductive_method(
        method=method,
        split_provider=lambda _name: selected,
        report_splits=["test"],
        metrics=["accuracy"],
        runtime=MethodEvaluationRuntime.from_features(features),
    )
