...
def predict_proba_from_bundle(
    ...
    *,
    ...
    from modssc.inductive.methods.helpers.classifier_bridge import detect_backend
...
class TorchBundlePredictMixin(ArgmaxPredictMixin):
    ...
    def predict_evaluation_proba(self, X: Any) -> Any:
        """Predict with the model variant declared by the fitted bundle."""

        meta = self._bundle.meta if self._bundle is not None else None
        predict_with_ema = bool(meta.get("predict_with_ema", False)) if meta else False
        return predict_proba_from_bundle(
            self._bundle,
            fitted_backend=self._backend,
            X=X,
            batch_size=int(self.spec.batch_size),
            use_ema=predict_with_ema,
        )

    def evaluation_metric_sets(self) -> Mapping[str, Mapping[str, Any]]:
        """Return native fitted reporting sets, when training produced them."""

        metric_sets = getattr(self, "evaluation_metric_sets_", None)
        return metric_sets if isinstance(metric_sets, Mapping) else {}
...
def get_torch_device(x: Any) -> Any:
    if isinstance(x, dict):
        ...
        for v in x.values():
            if hasattr(v, "device"):
                return v.device
        # Support custom containers that expose a device attribute directly.
        if hasattr(x, "device"):
            return x.device
