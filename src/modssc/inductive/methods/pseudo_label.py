...
class PseudoLabelMethod(InductiveMethod):
    ...
    def execution_contract(
        ...
        requires_unlabeled = str(spec.training_mode) == "joint_mlp"
        effective_capabilities = replace(
            capabilities,
            requires_unlabeled=requires_unlabeled,
        )
        optional_roles = () if requires_unlabeled else ("fit.X_u",)
        allow_empty_roles = () if requires_unlabeled else ("fit.X_u",)
        return with_inductive_input_roles(
            fallback_method_execution_contract(
                cls,
            ...
            optional_feature_roles=optional_roles,
            allow_empty_feature_roles=allow_empty_roles,
