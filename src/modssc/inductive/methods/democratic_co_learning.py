...
def _combine_scores_numpy(
    ...
    # Figure 2 filters weak learners, but it does not define an all-filtered
    # prediction.  Preserve the historical ModSSC fallback to the complete
    # ensemble so a valid fitted ensemble can never silently become class 0.
    if not np.any(eligible):
        eligible = np.ones_like(eligible, dtype=bool)
    effective_weights = weights * eligible
    # A degenerate confidence estimator can legitimately return three zero
    # lower/central estimates.  In that case only, use an unweighted vote.
    if not np.any(effective_weights > 0.0):
        effective_weights = eligible.astype(np.float64)
    scores = np.zeros((n_samples, n_classes), dtype=np.float64)
...
def _combine_scores_torch(preds_idx: Any, weights: Any, *, n_classes: int, min_confidence: float):
    ...
    if not bool(torch.any(weights_eff > 0.0)):
        weights_eff = eligible.to(weights.dtype)
    one_hot = torch.nn.functional.one_hot(preds_idx, num_classes=int(n_classes)).to(
        dtype=weights.dtype
    )
