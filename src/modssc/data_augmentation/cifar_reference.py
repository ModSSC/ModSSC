...
CIFAR_REFERENCE_AUGMENTER_ID = "vision.cifar_reference"
CIFAR_REFERENCE_CONTRACT_SCHEMA_VERSION = 2
...
def cifar_reference_runtime_identity(*, profile: str, seed: int) -> dict[str, Any]:
    """Return the canonical executable identity for one reference policy."""

    if not isinstance(seed, int) or isinstance(seed, bool):
        raise DataAugmentationValidationError("seed must be an integer.")
    resolved = resolve_cifar_augmentation_profile(profile)
    operation_names = (
        _GOOGLE_OPERATIONS if resolved == "google_fixmatch_ra" else _TORCHSSL_OPERATIONS
    )
    return {
        "schema_version": CIFAR_REFERENCE_CONTRACT_SCHEMA_VERSION,
        "augmenter_id": CIFAR_REFERENCE_AUGMENTER_ID,
        "implementation": ("modssc.data_augmentation.cifar_reference.CifarReferenceAugmentation"),
        "pixel_backend": {
...
class CifarReferenceAugmentation:
    ...
    def runtime_identity(self) -> dict[str, Any]:
        """Return the stable identity of the executable augmentation policy.

        Declarative benchmark metadata is intentionally excluded: this
        identity is derived from the object that will actually transform the
        batches.  Changing an output-affecting implementation detail requires
        incrementing ``CIFAR_REFERENCE_CONTRACT_SCHEMA_VERSION``.
        """

        identity = cifar_reference_runtime_identity(profile=self.profile, seed=self.seed)
        identity["config"] = {
            **identity["config"],
    ...
    def weak_batch(self, X: Any, *, indices: Any, sample_ids: Any, step: int) -> Any:
        """Implement the generic online-augmentation contract for labeled data."""
    ...
    def pair_batch(
        ...
        *,
        ...
        """Implement the generic online-augmentation contract for unlabeled data."""
