...
class LabelSpreadingSpec:
    """Configuration for Label Spreading (soft clamping).

    This matches the update described in "Learning with Local and Global Consistency":

        F_{t+1} = alpha * S * F_t + (1 - alpha) * Y

    where S is typically the symmetric normalized affinity matrix.
    """
...
def label_spreading_numpy(
    *,
    ...
    spec: LabelSpreadingSpec | None = None,
) -> DiffusionResult:
    if spec is None:
    ...
    for it in range(int(spec.max_iter)):
        SF = spmm_numpy(n_nodes=n_nodes, edge_index=edge_index, edge_weight=w_norm, X=F)
        F_new = a * SF + (1.0 - a) * Y
...
def label_spreading_torch(
    *,
    ...
    if torch is None:  # pragma: no cover
        raise ImportError("torch is required for label_spreading_torch")
    ...
    a = float(spec.alpha)
    for it in range(int(spec.max_iter)):
        SF = spmm_torch(n_nodes=n_nodes, edge_index=edge_index, edge_weight=w_norm, X=F)
        F_new = a * SF + (1.0 - a) * Y
...
class LabelSpreadingMethod(TransductiveMethod):
    info = MethodInfo(
    ...
    def __init__(self, spec: LabelSpreadingSpec | None = None) -> None:
        self.spec = spec or LabelSpreadingSpec()
        self._result: DiffusionResult | None = None

    def fit(self, data: Any, *, device: str | None = None, seed: int = 0) -> LabelSpreadingMethod:
        start = perf_counter()
        ...
        logger.info(
            "Label spreading sizes: n_nodes=%s labeled=%s",
            int(np.asarray(data.y).shape[0]),
            int(labeled_mask.sum()),
        )
        self._result = label_spreading(
    ...
    def predict_proba(self, data: Any) -> np.ndarray:
        if self._result is None:
            raise RuntimeError("LabelSpreadingMethod is not fitted yet. Call fit() first.")
