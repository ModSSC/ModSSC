# Laplace Learning — Calder et al. (2020)
...
Five active cards cover the Laplace Learning cells of MNIST Table 1 for one
...
It then draws balanced labels and solves the unnormalized harmonic system.

No author-generated VAE, neighbour bank, graph, or label permutation is loaded.
The VAE and graph are native, content-addressed stages: the first card builds
them and every method, label budget, and sampling seed with the same declaration
reuses them. Self-loops cancel from the Laplace operator `D - W`.
